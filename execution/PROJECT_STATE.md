...
- Last green commit: `246637c1bcd00e5e50a1e1b752346b52ff2c5722`
- Control-plane schema: `1.1.0`
- Updated at: `2026-09-23T19:10:00+08:00`

## Active gate: G6

G2 已 closed（`SO-0003`，2026-09-21）。G3 已 accepted（`SO-0004`，2026-09-22，E4）。G4 已 accepted（`SO-0005`，2026-09-22，E4）。G5 已 accepted（`SO-0006`，2026-09-22，E4）。G6 — 引導式介面工程整合現在為 active gate；`G6-UI-001`、`G6-QST-001`、`G6-AI-001`～`G6-AI-004` 保留歷史閉合紀錄。獨立反向審查確認 React 尚未實際經 HTTP 使用 Python IR 核心，因此新增 `G6-UI-002 — 補正真實 HTTP IR 介面整合` 為唯一在製項目。

G6-UI-002 控制面補正（2026-09-23）：依藍圖禁止重開 done 項目的規則，保留 G6-UI-001 歷史並新增 corrective item。固定範圍是 maturity 不變量、127.0.0.1 HTTP IR seam、React 真實 adapter 與 browser/direct HTTP byte-equivalence；database、job lifecycle、CAD 與 package 留在其正式後續邊界。G6-A11Y-001 已完成的工具與歷史證據保留，但狀態退回 planned，待新 UI 固定後重新執行。

G6-UI-002 maturity 補正（2026-09-23）：`megis.guides.flow` 不再直接宣告 guided IR 為 `PROTOTYPE`，而是以 G3 maturity evaluator 根據現有證據計算。由於 critical unknown、layout、geometry、rules、drawing QA 與 package reproducibility 尚未閉合，guided IR 現為 `DRAFT`；v2 Engineering IR schema 補回藍圖定義的 `DRAFT` 狀態，相關 G1／G3／G6 tests 全綠。

G6-UI-002 HTTP contract（2026-09-23）：新增無第三方依賴的同步本機 API，固定綁 `127.0.0.1:4174`，提供 health、真實 capability manifest、真實 guided questions 與 canonical IR draft。Host／Origin／Content-Type／64 KiB body／5 秒 timeout／correlation ID 皆有 fail-closed contract；HTTP response bytes 已與直接 Python API bytes 比對。此段無 persistence、job、CAD 或 package。

G6-UI-002 已閉合（2026-09-23）：React production flow 已改用真實本機 HTTP adapter，API 離線時 fail closed 且不退回 synthetic data；實際 Chrome 操作取得 `DRAFT 2.0.0`、correlation ID、4 個 components 與 2 個 unsafe unknowns。瀏覽器顯示的 canonical response SHA-256 與 direct HTTP 同輸入重算值同為 `71674fdf...a801`；`artifacts/g6-ui-002/verification.json` 四項檢查全綠（E3），實作 commit `9d3278c`。`G6-A11Y-001` 接續 in_progress，必須針對新 UI 重跑自動化與真人抽查。

G6-UI-001 已閉合（2026-09-22，歷史）：capability-driven guided flow 的 schema、golden、Python builder 與當時 synthetic adapter 證據保留於 `artifacts/g6-ui-001/verification.json` 及完工報告；舊 adapter 已由 G6-UI-002 真實 HTTP adapter 取代。`tests/test_g6_ui_001.py` 11 tests + `scripts/verify_g6_ui_001.py` E3 allChecksPassed（golden fingerprint `7bfc576a...`）。
...
G6-A11Y-001 自動化準備段已完成（2026-09-22；2026-09-23 因 G6-UI-002 補正改回 planned）：加入 `axe-core 4.10.3`，五個主要路由以 WCAG 2.2 A／AA tags 掃描為 0 violations；8 組 palette 對比度、skip link、route focus、aria-current、行動選單 Escape／Tab wrap、aria-pressed 等共 9 tests 全綠。另以實際 Chrome（1536×729）完成 8 項代理自動化瀏覽器稽核，證據為 `artifacts/g6-a11y-001/browser-audit.json`。人工證據契約補上 Draft 2020-12 schema、12 個 canonical checks、版本化空白範本與 fail-closed verifier；工具與歷史證據均保留，新 UI 固定後必須重跑，真人鍵盤與螢幕閱讀器仍 pending，`closureEligible: false`。
...
2026-09-23 全面盤點同步（`246637c`）：收尾上階段 blocker 可視化段落並 push；啟動 `http://127.0.0.1:4173/progress`；以完整 baseline（617 Python + 25 Frontend + control-plane／secret／artifact／toolchain／maturity 全綠）確認 `246637c` 為新綠色基線。`G6-A11Y-001` 仍為唯一 in_progress，等待具名真人鍵盤與螢幕閱讀器簽錄（`B-G6-A11Y-HUMAN-001`），後續 USE→E2E→REV→ACC 依賴鏈未提前。
...
| ID | Status | Evidence |
|---|---|---|
| G6-UI-001 | done | schemas/v3/capability-manifest.schema.json、contracts/g6/golden/capability-manifest.json、megis/guides/、tests/test_g6_ui_001.py、scripts/verify_g6_ui_001.py、artifacts/g6-ui-001/verification.json；歷史 synthetic adapter 已由 G6-UI-002 取代 |
