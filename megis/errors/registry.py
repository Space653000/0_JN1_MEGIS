...
_ERROR_CODES: list[ErrorCode] = [
    ...
    ErrorCode("MEGIS-AI-001", ErrorDomain.AI, 1, ErrorSeverity.ERROR, True, "AI 服務目前不可用，已切換回表單流程。", "AI provider unavailable, timed out, or failed; deterministic form fallback required"),
    ErrorCode("MEGIS-AI-002", ErrorDomain.AI, 2, ErrorSeverity.ERROR, False, "AI 輸出格式無效，內容未被採用。", "schema-invalid AI model output must be rejected without repair"),
    ErrorCode("MEGIS-AI-003", ErrorDomain.AI, 3, ErrorSeverity.ERROR, True, "AI 使用上限已到，已切換回表單流程。", "AI call, token, output, or cost limit exceeded; deterministic form fallback required"),
