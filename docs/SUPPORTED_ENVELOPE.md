...
V3 目標案例為外形 `120 × 80 × 35 mm`、兩片 PCB、USB-C、M3 removable cover；在 `G1-REQ-001` 完成公開來源研究、`G1-ENV-001` 建立機器設定並完成一致性測試前，不視為已驗證 envelope。

## 變更規則

Envelope 變更需要 ADR、sign-off、golden／boundary／negative regression、UI capability filter 與 maturity 重算。`config/envelope/` 目前只保存狀態說明，尚未冒充完成的機器設定。
