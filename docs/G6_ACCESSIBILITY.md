...
`G6-A11Y-001` 目前為 `planned`，依賴 `G6-UI-002`。既有自動化、Chrome 代理稽核、schema、紀錄器與 verifier 均保留為歷史／準備證據；由於 React 將改接真實 HTTP adapter，新 UI 固定後必須重新執行自動化、瀏覽器及真人鍵盤／螢幕閱讀器抽查。在完整新證據形成前不得標記完成，也不得解鎖 G6-USE-001。
