...
目前位於 **G0：基礎建設與可行性**；`G0-REP-001`、`G0-ENV-001`、`G0-CAD-001` 與 `G0-DRW-001` 已完成，唯一在製項目為 `G0-SIM-001`。
...
UI-0 已完成驗收，但仍只是使用者體驗原型；G0 不會把其合成展示資料當作工程輸出。CadQuery 可行性已通過，FreeCAD 圖面路徑採固定模板 SVG fallback；目前正在盤點 COMSOL 可行性。最新狀態與驗證證據：
