...
**G0 與 G1 沿用既有驗收，G2 為 active Gate**。`V3C-DET-001` 已完成 semantic fingerprint policy 1.0.0、STEP/DXF 正規化、binary STL 與 replay 測試；目前唯一在製項目為 `V3C-MAT-001`，修正 feasibility spike 的 classification/maturity 並建立 repository-wide scanner。既有 done 項目與 accepted Gate 不重開。
