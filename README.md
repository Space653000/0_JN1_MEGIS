...
**G0～G5 已完成 Gate acceptance，G6 為 active Gate**。V3 必要工作圖共有 74 項，目前控制面為 **61 done、1 in progress、12 planned**；唯一在製項目是 `G6-AI-004 — AI grounded explanation`。
...
- 最新完整 baseline：570 個 Python tests、12 個 frontend tests，以及 control-plane、secret、artifact、toolchain、maturity、lint、typecheck、build 全綠。
...
1. `G6-AI-004`：只依 IR、規則結果與 manifest 的 grounded explanation。
2. `G6-A11Y-001`：WCAG 2.2 AA 自動化與人工抽查。
...
| AI 輔助 | adapter、schema-bound Intent 萃取與 54-case KPI 已驗證；預設關閉，故障退回表單，未確認值隔離 | 尚未接真實 provider；grounded explanation 待施工 |
