  ...
  return <>
    ...
    <section className="wizard-grid">
      <div className="form-stack">
        ...
        <article className="panel form-card"><div className="section-title"><CircuitBoard size={20} /><div><h2>內部零件</h2><p>選項只來自後端已證明的能力清單。</p></div></div><div className="choice-row" role="group" aria-label="內部零件選擇">
          <button type="button" aria-pressed={input.pcbCount === 1} className={`choice-card ${input.pcbCount === 1 ? "selected" : ""}`} onClick={() => update({ pcbCount: 1 })}><CircuitBoard size={21} /><strong>1 片 PCB</strong><span>單板配置</span></button>
          <button type="button" aria-pressed={input.pcbCount === 2} className={`choice-card ${input.pcbCount === 2 ? "selected" : ""}`} onClick={() => update({ pcbCount: 2 })}><CircuitBoard size={21} /><strong>2 片 PCB</strong><span>參考案例</span></button>
