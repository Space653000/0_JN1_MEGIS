  ...
  return <>
    ...
    <section className="content-grid">
      ...
      <div className="right-stack">
        <article className="panel current-work"><div className="panel-heading"><div><span className="panel-kicker">目前工作項目</span><h2>{current?.id}</h2></div><span className="status-pill active">施工中</span></div><h3>{current?.title}</h3><p>施工狀態由唯一 JSON 資料來源與資料結構驅動；畫面不另存狀態副本。</p><div className="acceptance-list">{current?.acceptance.map((item) => <div key={item}><span className="empty-check" />{item}</div>)}</div><div className="work-footer"><span><GitBranch size={16} /> 基準 {state.baselineCommit.slice(0, 7)}</span><button type="button">查看驗收條件 <ArrowRight size={16} /></button></div></article>
