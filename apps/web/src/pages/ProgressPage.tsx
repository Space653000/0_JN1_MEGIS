...
export function ProgressPage() {
  ...
  const currentBlockers = state.blockers.filter((blocker) => current?.blockedBy.includes(blocker.id));
  ...
  return <>
    ...
    <section className="content-grid">
      ...
      <div className="right-stack">
        <article className="panel current-work"><div className="panel-heading"><div><span className="panel-kicker">目前工作項目</span><h2>{current?.id}</h2></div><span className={`status-pill work-${current?.status}`}>{current ? workLabels[current.status] : "未知"}</span></div><h3>{current?.title}</h3><p>施工狀態由正式 WORK_QUEUE 控制面與驗證器驅動；畫面不另存狀態副本。</p><div className="acceptance-list">{current?.acceptanceResults.map((result) => <div className={result.status} key={result.criterion}>{result.status === "passed" ? <CheckCircle2 className="acceptance-check" size={17} /> : <span className="empty-check" />}{result.criterion}</div>)}</div>{currentBlockers.length > 0 && <section className="work-blockers" aria-labelledby="current-blockers-heading"><AlertTriangle size={19} /><div><h4 id="current-blockers-heading">等待真人驗收</h4>{currentBlockers.map((blocker) => <div className="work-blocker" key={blocker.id}><strong>{blocker.title}</strong><span>責任者：{blocker.owner}</span><p>{blocker.fallback}</p></div>)}</div></section>}<div className="work-footer"><span><GitBranch size={16} /> 基準 {state.baselineCommit.slice(0, 7)}</span><button type="button" aria-expanded={acceptanceOpen} aria-controls="acceptance-evidence" onClick={() => setAcceptanceOpen((open) => !open)}>{acceptanceOpen ? "收合驗收條件" : "查看驗收條件"} <ArrowRight className={acceptanceOpen ? "disclosure-arrow open" : "disclosure-arrow"} size={16} /></button></div></article>
        {acceptanceOpen && <article className="panel acceptance-panel" id="acceptance-evidence">
          <div className="panel-heading"><div><span className="panel-kicker">目前工作驗收證據</span><h2>完成條件與責任邊界</h2></div><span className={`status-pill work-${current?.status}`}>{current ? workLabels[current.status] : "未知"}</span></div>
          <div className="acceptance-evidence-list">{current?.acceptanceResults.map((result) => <section key={result.criterion}>
            <div className={`acceptance-state ${result.status}`}>{result.status === "passed" ? <CheckCircle2 size={18} /> : <CircleDashed size={18} />}<strong>{result.criterion}</strong><span>{result.status === "passed" ? "已通過" : "待施工驗證"}</span></div>
            {result.evidence.length > 0
              ? <ul>{result.evidence.map((path) => <li key={path}><code>{path}</code></li>)}</ul>
              : <p>{current.evidence.length > 0 ? "支援材料已備妥，但完整人工驗收證據尚未形成，因此不會自動標記為完成。" : "此條件尚未形成可重跑證據，因此不會自動標記為完成。"}</p>}
          </section>)}</div>
          {current && current.evidence.length > 0 && <section className="prepared-evidence" aria-labelledby="prepared-evidence-heading">
            <h3 id="prepared-evidence-heading">已備妥的支援證據</h3>
            <p>以下材料可協助驗收，但不等於 acceptance 已通過。</p>
            <ul>{current.evidence.map((path) => <li key={path}><code>{path}</code></li>)}</ul>
          </section>}
          {current && (current.verificationCommands ?? []).length > 0 && <section className="verification-commands" aria-labelledby="verification-commands-heading">
            <h3 id="verification-commands-heading">下一步驗證指令</h3>
            <p>請由實際執行者完成真人紀錄後再執行；pending 結果會 fail closed。</p>
            <ul>{current.verificationCommands?.map((command) => <li key={command}><code>{command}</code></li>)}</ul>
          </section>}
          <footer className="acceptance-boundary"><AlertTriangle size={18}/><p><strong>{current?.gate} 尚未完成。</strong>只有目前 Gate 的全部退出條件具有證據後，才能進入下一關卡。</p></footer>
        </article>}
        <article className="panel queue-panel"><div className="panel-heading"><div><span className="panel-kicker">工作佇列</span><h2>後續工作</h2></div><span>{state.workItems.length} 個項目</span></div><div className="queue-table" role="table" aria-label="後續工作清單"><div className="sr-only" role="row"><span role="columnheader">工作編號</span><span role="columnheader">標題</span><span role="columnheader">狀態</span></div>{state.workItems.filter((item) => item.id !== state.currentWorkItem).map((item) => <div className="queue-row" role="row" key={item.id}><span role="cell" className="queue-id">{item.id}</span><span role="cell" className="queue-title">{item.title}</span><span role="cell" className={`status-pill work-${item.status}`}>{workLabels[item.status]}</span></div>)}</div></article>
