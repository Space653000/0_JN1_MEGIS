...
import { EngineeringApiError, localEngineeringApi } from "./adapters/engineering-api-adapter";
...
export function App() {
  ...
  const [draft, setDraft] = useState<IrDraftResponse | null>(null);
  ...
  useEffect(() => {
    let active = true;
    localEngineeringApi.loadCatalog().then((next) => {
      if (active) { setCatalog(next); setCatalogError(null); }
    }).catch((error: unknown) => {
      if (active) setCatalogError(error instanceof EngineeringApiError ? error.message : "本機工程 API 無法使用。");
    });
    return () => { active = false; };
  }, []);

  const createDraft = async (mode: "reference_only" | "provided" | "unknown") => {
    const response = await localEngineeringApi.createIrDraft(model.input, mode);
    setDraft(response);
    go("/review");
  };
  ...
  if (path === "/design" || path === "/") page = <DesignPage model={model} setModel={setModel} catalog={catalog} catalogError={catalogError} onNext={createDraft} />;
  if (path === "/review") page = <ReviewPage draft={draft} onBack={() => go("/design")} onRun={() => go("/run")} />;
  if (path === "/run") page = <RunPage onComplete={() => go("/results")} />;
  if (path === "/results") page = <ResultsPage draft={draft} onRestart={() => go("/design")} />;
